...
namespace KafKaService
{
    public class KafkaProducer : IDisposable
    {
        ...
        {
            Publish(new KafkaMessageModel() { Key = key, Message = message, Partition = partition, Topic = topic }, callback);
        }
        /// <summary>
        /// 发送消息
        /// </summary>
        ...
        /// <param name="callback"></param>
        public void Publish(KafkaMessageModel kafkaMessage, Action<DeliveryResult> callback = null)
        {
        ...
        {
            return await PublishAsync(new KafkaMessageModel() { Key = key, Message = message, Partition = partition, Topic = topic });
        }
        /// <summary>
        /// 异步发送消息
        /// </summary>
        /// <param name="kafkaMessage"></param>
        /// <returns></returns>
        public async Task<DeliveryResult> PublishAsync(KafkaMessageModel kafkaMessage)
        {
            if (string.IsNullOrEmpty(kafkaMessage.Topic))
            {
                throw new ArgumentException("topic can not be empty", nameof(kafkaMessage.Topic));
        ...
        public static KafkaProducer Create(KafkaBaseOptionsModel kafkaBaseOptions)
        {
